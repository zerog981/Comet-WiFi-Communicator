...
def send_config_data( # nosec B107
    ...
    if logger:
        logger.info(f"SSID: {wifi_ssid}, MQTT Broker: {mqtt_server_ip}:{mqtt_port}, Comet Endpoint: {comet_wifi_ip}:{comet_wifi_port}")
    ...
    data = wifi_ssid_hex + string_to_unicode(separator, uppercase=True) + wifi_password_hex + separator + mqtt_user \
           + separator + mqtt_password + separator + mqtt_server_hex + separator \
           + mqtt_port_hex
