# CHANGELOG

<!-- version list -->

## v1.0.0 (2026-09-13)

### Bug Fixes

- Bandit exception
  ([`d5306b0`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/d5306b054e06c3898dd8637fe98ac486d2f2ae0e))

- Improve cli
  ([`b6922f1`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/b6922f1c0afdb75dc63096bfe16c4f675878a9f9))

- Remove unused code
  ([`eae1cb1`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/eae1cb16f50081a6c093aa7fa067ded6360debb7))

### Chores

- Update pipeline
  ([`9020e20`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/9020e20fcc2159747d5813d048418d8e74a745b2))

- Update pipeline
  ([`78deb94`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/78deb94891a63bd939501e967184444e646fb2d3))

- Update pipeline
  ([`2aab1dd`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/2aab1dd5e9b0d19bbd8c95496da083f86c0bdbfe))

### Features

- Provisioning modules and tests
  ([`4dd0bfd`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/4dd0bfd39d9bdcecc5542a67e61a7bcb0b300a26))

- Release the setup tool as comet-wifi-communicator 1.0.0
  ([`439e149`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/439e149277c81f836efddb1e1ebaf88b88ce2afd))

- Typing and module importing
  ([`3827ba2`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/3827ba254e87c793f72e897cd19213d39e820cd2))

- Update readme
  ([`bbd7adb`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/bbd7adbb0ef9364a01476943db0c77083702e76e))

### Breaking Changes

- The MQTT client (Thermostat, paho-mqtt) is gone, use aiocometwifi instead.


## v0.8.0 (2026-08-06)

### Features

- Add details to documentation
  ([`760cc21`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/760cc211239b3a60cf88884d55d574c2058105c5))

- Add documentation
  ([`1447bfb`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/1447bfbecb085227b31b05d7663dbc76e05c5d4e))

- Add implementation details hint
  ([`4f1c937`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/4f1c9375f8e1d5ea502e5789f0e97ae2bd34525d))

- Add missing docstrings
  ([`2b027d2`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/2b027d2efd5014144eba704249f8134b45f1ba0c))

- CLI command for setting up thermostat
  ([`097d042`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/097d0420a79b7348746296369736434c88168d30))

- Improve documentation
  ([`c621dee`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/c621deece1fa923a4744b00d87463ef28a76d338))


## v0.7.1 (2026-06-16)


## v0.7.0 (2026-06-16)

### Features

- Add cli command for setting up thermostat
  ([`8cb68c3`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/8cb68c34224f21c80c1af4fef2242d9efb7f8413))


## v0.6.0 (2026-06-03)

### Bug Fixes

- Add CI to development pull requests
  ([`d066f92`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/d066f9263124d0fa2c972b2fb622817f9ba80639))

- Add ruff to requirements
  ([`b756b58`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/b756b5815f8fff8dea5db429e641a4517e29d6d7))

- Correctly use updated helper functions
  ([`f9f7691`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/f9f76914d169be081ffdb738329921aff7720b34))

- Coverage test
  ([`45a949e`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/45a949e3541b28cf892882f573445a7913bd7697))

- Failure check in MQTT connect
  ([`3c55f0f`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/3c55f0f85d028b1bde9e7c30a53dfe54905d0926))

- File structure
  ([`0157605`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/01576050091fb5941633f93ada503c154335d8d5))

- Pipeline for testing
  ([`bebf9bc`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/bebf9bc22a12624b299a48c34bae589e82aaeedd))

- Remove install steps
  ([`c4a6f0a`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/c4a6f0a4af37031ffa6a4a70bd3050f018dff2cd))

- Test path
  ([`9b9d11e`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/9b9d11e28f1c6ec7d59ba637c32329f6191dad6e))

- Test variable
  ([`e1d3e94`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/e1d3e9440c96eeb7942f248f9125bd7e962112b9))

- Testing imports
  ([`96833eb`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/96833eb2ce4035c6fff2660a316f2a0c9b8e3184))

- Tests for MQTT on_connect
  ([`1b073a8`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/1b073a8b6998cd861cd169a255f5e8487322669e))

- Undefined logger on error
  ([`44aa7a1`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/44aa7a1b431684b5a658826a18632d199eeb150a))

### Features

- Add config consts
  ([`f1c20ca`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/f1c20caabc9381d592e514da837bd2cff47cc724))

- Add happy path test for thermostat setup
  ([`e1ce53c`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/e1ce53cf95c9d89a9c0fda0b858c7f62914bf91e))

- Add parameters for window open configuration
  ([`8df3abb`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/8df3abb007097a4277c4d6b0e0f9800176b593e1))

- Add test for ThermostatData class
  ([`2f9831b`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/2f9831b2d99695fa9ff62dd0d03b92f215dc7acc))

- Add tests for char_to_unicode helper
  ([`e4f4eed`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/e4f4eed04d8c4fd0faa1e08a8c6183e92c3cf2a8))

- Add tests for encodeTemperature helper
  ([`8d11fc9`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/8d11fc9d8a57bbf699dbd32a9f4361a5f2038975))

- Add tests for helper fuctions
  ([`44dbf1c`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/44dbf1cedf899c7052c0355b027196f124967f0d))

- Add tests for int_to_hex_str helper
  ([`dee2c91`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/dee2c91fc45c042362d09da7ab5ffe305120fa1a))

- Add tests for ip_to_hex_str
  ([`110b045`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/110b0451e130c8296b53a8986685c3801dbc7fda))

- Add tests for receiving configuration data
  ([`735c125`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/735c1258c61f41206893a708e7dc2b7d3c6ebcb2))

- Add tests for ThermostatConfig
  ([`6d1366d`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/6d1366d3208686e93dacc417af12a07d2c882a45))

- Add tests for validate_and_streamline_mac
  ([`4849022`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/4849022e1387fb440605736068e9382239992241))

- Add thermostat setup function
  ([`ce692bf`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/ce692bf1c840a117b256114a14c908ccf03be8ff))

- Test connection to different thermostat IP
  ([`4c0e7c7`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/4c0e7c7c72f1b74abfbf5fd540cecf813aa5c081))

- Test for socket closing
  ([`d70318c`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/d70318c0d8ad6c02fceb4757b38b04edfa200672))

- Test for socket timeout
  ([`de126bc`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/de126bc6c2e8b3cd3709e203ab58700aa1da64c7))

- Tests for data formatting
  ([`f8ed270`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/f8ed270e69d440f1e423497ba8f4d270fb747e5f))

- Tests for logger
  ([`81600e3`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/81600e36aea20b20872aa4d963fc6b8cf5fe9538))

- Use dataclass instead of dict
  ([`2cbb885`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/2cbb8854d19f843c75074897dbba2b7389b9edd3))

- Write and read configuration from device
  ([`2caee19`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/2caee199628028598def7d48229b583fcf99435d))


## v0.5.2 (2026-04-22)


## v0.5.1 (2026-04-22)

### Bug Fixes

- Purge pip cache before check
  ([`a935b9e`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/a935b9e0fd0dc8f4dc05f9488c39a9817ca3c343))


## v0.5.0 (2026-04-22)

### Features

- Add sleep before checking published package
  ([`d6d9225`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/d6d9225428ec76b5c47721ba2c4aadc01091b7ab))


## v0.4.8 (2026-04-22)

### Bug Fixes

- Variable usage
  ([`8af698d`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/8af698d9a00e24ef1a3d0152a5eb767303af216b))


## v0.4.7 (2026-04-22)

### Bug Fixes

- Publishing condition
  ([`683b4d5`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/683b4d592a836919d2e89fba4833b8b60381639f))


## v0.4.6 (2026-04-22)

### Bug Fixes

- PIP not using latest index
  ([`030ba81`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/030ba81846a56a1c4830f2c509fdaf9f2e3be2ff))


## v0.4.5 (2026-04-22)

### Bug Fixes

- Version variable
  ([`b2fc685`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/b2fc685dce5fd696ce59e9364f9af5d968da7193))


## v0.4.4 (2026-04-22)

### Bug Fixes

- Permissions
  ([`723198e`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/723198e2065251e32cacfcb6dd6fe99b66674393))


## v0.4.3 (2026-04-22)

### Bug Fixes

- Dependencies
  ([`4edddd4`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/4edddd4a19c594427e70fb71a2bcce3dc671e01e))

- Missing checking-out of repository
  ([`5fa474e`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/5fa474e20a4ff9438561f340a59a4fa08dd58b82))


## v0.4.2 (2026-04-22)

### Bug Fixes

- Missing python installation in publishing jobs
  ([`6829915`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/6829915dd521bb9bc4bac559836ae8eda4fc08de))


## v0.4.1 (2026-04-22)

### Bug Fixes

- Condition in publishing steps
  ([`7610604`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/7610604fa72e79fdd3a60345ff870a957083dcc8))


## v0.4.0 (2026-04-22)

### Bug Fixes

- Job condition
  ([`b40f424`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/b40f424d6acdba5d08358ad2b0682ce721f00685))

### Features

- Separate jobs
  ([`2c5f16a`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/2c5f16ae0a85d341137e6ee32bee70fd13058fc1))


## v0.3.0 (2026-04-22)

### Features

- Use trusted publisher instead of token
  ([`17ca6a6`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/17ca6a634cc54f1090e73bbcbeb35fccaf58e3b0))


## v0.2.4 (2026-04-22)


## v0.2.3 (2026-04-22)

### Bug Fixes

- Wrong package
  ([`c9a6007`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/c9a6007c9116f7708a368fcc928c6d7ee224fab2))


## v0.2.2 (2026-04-22)

### Bug Fixes

- Typo
  ([`e020fd6`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/e020fd6bc2f9f063a956acb59757263c652475ce))


## v0.2.1 (2026-04-22)

### Bug Fixes

- Variable in project description
  ([`f221bb8`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/f221bb8002a1a8569971dc76d8eea7531d2fe482))


## v0.2.0 (2026-04-22)

### Features

- Check installed version is most recent
  ([`e492f52`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/e492f525ecf6567994db13d8c3b8d881732e5f7e))


## v0.1.0 (2026-04-22)

### Bug Fixes

- Missing version in cd
  ([`123386b`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/123386bbc9f24ac67d9a04696ce65a04d9461bdd))

- Typo
  ([`590772a`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/590772accd11b27c18eee6f0cbaeeb90f6b93371))

- Typo in cd condition
  ([`98da947`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/98da94708465f9d74ed42edccb799d283dc164e6))

- Typo in cd condition
  ([`d5909e4`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/d5909e428e18c11cf6f5cd346171b53138422804))

### Features

- Add release to PyPI
  ([`9cf1c24`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/9cf1c24763a1dc41dda583e4a6058e20f7108647))

- Automatically release to TestPyPI
  ([`38cc78a`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/38cc78ac3593e9f279ca18f2b8aefabcc11e3f32))


## v0.0.9 (2026-04-20)

### Bug Fixes

- Allow zero versions for semantic_release
  ([`443ea8f`](https://github.com/zerog981/Comet-WiFi-Communicator/commit/443ea8f63cb5f23eaf279762d64283a6535b7389))


## v0.0.8 (2026-04-20)


## v0.0.7 (2026-04-13)


## v0.0.6 (2026-04-11)


## v0.0.5 (2026-04-11)


## v0.0.4 (2026-04-07)


## v0.0.3 (2026-03-27)


## v0.0.2 (2026-03-27)


## v0.0.1 (2026-02-04)

- Initial Release
